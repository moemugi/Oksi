...
export default function Statistics() {
  ...
  // Environmental statistics for charts
  const avgTemp =
    sensorData.length > 0
      ? (
          sensorData.reduce((sum, d) => sum + (d.temperature || 0), 0) /
          sensorData.length
        ).toFixed(1)
      : "--";
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      // computaiton for stats in report generation
      ...
      const avgSoilVal = statsData.length
      ...
      if (avgTempVal < cropThreshold.tempMin || avgTempVal > cropThreshold.tempMax)
      ...
      if (lightLux < cropThreshold.lightMin || lightLux > cropThreshold.lightMax)
        issues.push("Light Intensity");
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={styles.filters}>
          ...
          />
          <Dropdown label={t.crop} options={cropOptions} value={crop} onChange={setCrop} />
        </View>
